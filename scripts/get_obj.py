#!/usr/bin/env python
"""
This is for object detection on Pepper
"""
...
def camera2pose(depth, pixel_x, pixel_y, target_frame):

    # x, y, z direction are relative to robot frame (unit: meters)
    obj_depth_frame = PoseStamped()
    obj_depth_frame.header.frame_id = "CameraDepth_frame"
    ...
    if target_frame != "CameraDepth_frame":
...
def create_ros_marker(input_obj_array, marker_scale=0.5, red=1.0, green=1.0, blue=0.0, alpha=1.0):  # input point list
...
def bd_callback(data):
    ...
    for obj in data.bounding_boxes:
        ...
        if obj.Class != 'person':
            ...
            obj_x, obj_y = camera2pose(depth_val, obj_pixel_x, obj_pixel_y, "CameraDepth_frame")  # unit: meter
            ...
            if obj_buffer.Class == '' and obj_buffer.x == 0.0 and obj_buffer.y == 0.0:  # empty buffer
                obj_buffer = temp_obj
            ...
            obj_list.append(temp_obj)  # record all the object, not for single human detection

            # Convert points to map frame
            glob_obj_pose = ObjPose()
            glob_obj_pose.Class = obj.Class
            glob_obj_pose.x, glob_obj_pose.y = camera2pose(depth_val, obj_pixel_x, obj_pixel_y, "map")  # unit: meter
    ...
    obj_marker = create_ros_marker(glob_obj_list)
...
if __name__ == '__main__':
    ...
    rospy.loginfo('fx, fy, cx, cy = %.4f, %.4f, %.4f, %.4f' % (fx, fy, cx, cy))
    ...
    # Publisher
    depth_box_pub = rospy.Publisher("/get_obj/depth_box", Image, queue_size=1)
    temp_map_pub = rospy.Publisher("/get_obj/temp_map", Image, queue_size=1)
    obj_pose_pub = rospy.Publisher("/get_obj/obj_pose", ObjPoseArray, queue_size=10)
    obj_mark_pub = rospy.Publisher('/get_obj/obj_marker', Marker, queue_size=1)
    ...
    while not rospy.is_shutdown():
        try:
            ...
            rospy.loginfo('Shut down get objects ...')
