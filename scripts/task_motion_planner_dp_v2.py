...
class TaskMotionPlannerDP(TaskMotionPlannerFCFS):
    ...
    def check_robot_state(self, (cur_x, cur_y, cur_yaw)):
        # charng current position to state
        rospy.loginfo('cur_location: {0}, {1}, {2}'.format(cur_x, cur_y, cur_yaw))
    ...
    def run_plan(self):
        ...
        while not rospy.is_shutdown():
            self.check_robot_state(get_cur_pos(pos_topic='/amcl_pose', t=0.2))
            self.plan_motion()
            rate.sleep()
...
if __name__ == '__main__':
    ...
    tamp.run_plan()  # this is for simulation
